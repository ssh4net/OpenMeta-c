...
static ByteVec
build_transfer_target_dng_fixture(const char* existing_creator_tool)
{
    static const char make[] = "Canon";
    std::array<unsigned char, 1024> file {};
    const std::string xml = (existing_creator_tool != nullptr)
    ...
    append_u16le(file.data(), &size, 42U);
    append_u32le(file.data(), &size, 8U);
    entry_count = (existing_creator_tool != nullptr) ? 3U : 2U;
    append_u16le(file.data(), &size, entry_count);
    ...
    append_u16le(file.data(), &size, 0xC612U);
    append_u16le(file.data(), &size, 1U);
    append_u32le(file.data(), &size, 4U);
    append_u8(file.data(), &size, 1U);
    append_u8(file.data(), &size, 6U);
    append_u8(file.data(), &size, 0U);
    append_u8(file.data(), &size, 0U);
...
static openmeta::XmpExistingSidecarPrecedence
...
static openmeta::XmpExistingDestinationEmbeddedPrecedence
to_cpp_existing_embedded_precedence(
    omc_transfer_existing_xmp_precedence precedence)
{
    switch (precedence) {
    case OMC_TRANSFER_EXISTING_XMP_PREFER_EXISTING:
                              ...
                              TransferExecuteParitySummary* out)
{
    ...
    omc_store existing_sidecar_store;
    omc_store existing_embedded_store;
    ...
    *out = TransferExecuteParitySummary {};
    ...
    if (options.existing_sidecar_creator_tool != nullptr
        ...
        prepare_opts.existing_sidecar_xmp_precedence
            = options.existing_sidecar_precedence;
    }
    if (options.merge_existing_embedded) {
        if (!read_omc_store(target_bytes, &existing_embedded_store)) {
            if (have_existing_sidecar) {
                omc_store_fini(&existing_sidecar_store);
            }
            omc_arena_fini(&sidecar_out);
            omc_arena_fini(&edited_out);
            omc_store_fini(&source_store);
                              ...
                              TransferExecuteParitySummary* out)
{
    ...
    *out = TransferExecuteParitySummary {};
    ...
    if (options.existing_sidecar_creator_tool != nullptr
        ...
        exec_opts.prepare.xmp_existing_sidecar_mode
            = openmeta::XmpExistingSidecarMode::MergeIfPresent;
        exec_opts.prepare.xmp_existing_sidecar_precedence
            = to_cpp_existing_sidecar_precedence(
                options.existing_sidecar_precedence);
    ...
    if (options.merge_existing_embedded) {
        ...
        exec_opts.prepare.prepare.xmp_conflict_policy
            = openmeta::XmpConflictPolicy::ExistingWins;
        exec_opts.xmp_existing_destination_embedded_mode
            = openmeta::XmpExistingDestinationEmbeddedMode::MergeIfPresent;
        exec_opts.xmp_existing_destination_embedded_precedence
            = to_cpp_existing_embedded_precedence(
                options.existing_embedded_precedence);
        exec_opts.prepare.xmp_existing_destination_embedded_mode
            = openmeta::XmpExistingDestinationEmbeddedMode::MergeIfPresent;
        exec_opts.prepare.xmp_existing_destination_embedded_precedence
            = to_cpp_existing_embedded_precedence(
                options.existing_embedded_precedence);
                              ...
                              TransferPersistParitySummary* out)
{
    ...
    *out = TransferPersistParitySummary {};
    ...
    if (options.persist_stage_output
        && !write_file_bytes(output_path, target_bytes)) {
        if (have_existing_embedded) {
            omc_store_fini(&existing_embedded_store);
        }
        if (have_existing_sidecar) {
    ...
    if (options.persist_staged_sidecar_creator_tool != nullptr
        && !write_file_bytes(sidecar_path,
                             build_transfer_xmp_sidecar_fixture(
                                 options.persist_staged_sidecar_creator_tool))) {
        if (have_existing_embedded) {
            omc_store_fini(&existing_embedded_store);
        }
        if (have_existing_sidecar) {
    ...
    if (options.persist_staged_sidecar_creator_tool == nullptr
        && (options.existing_sidecar_creator_tool != nullptr
            || options.existing_sidecar_xmp_builder != nullptr)
        && !write_file_bytes(sidecar_path, existing_sidecar_bytes)) {
        if (have_existing_embedded) {
            omc_store_fini(&existing_embedded_store);
        }
        if (have_existing_sidecar) {
    ...
    if (options.persist_use_explicit_sidecar_base_path) {
        persist_opts.xmp_sidecar_base_path = sidecar_base_path.c_str();
    }
                              ...
                              TransferPersistParitySummary* out)
{
    ...
    *out = TransferPersistParitySummary {};
    ...
    if (options.persist_use_explicit_sidecar_base_path) {
        exec_opts.xmp_sidecar_base_path = sidecar_base_path;
    } else {
        exec_opts.xmp_sidecar_base_path = output_path;
    }
    exec_opts.execute.edit_apply = true;
                                 ...
                                 bool compare_output_bytes)
{
    ...
    ok = compare_text_field(case_name, "cleanup_status", omc.cleanup_status,
                            cpp.cleanup_status)
         && ok;
    if (compare_output_bytes && omc.output_bytes != cpp.output_bytes) {
        std::fprintf(stderr, "%s mismatch in output_bytes\n", case_name);
        std::fprintf(stderr, "  omc: %llu\n",
                     (unsigned long long)omc.output_bytes);
        std::fprintf(stderr, "  cpp: %llu\n",
                     (unsigned long long)cpp.output_bytes);
        ok = false;
    }
    ok = compare_bool_field(case_name, "sidecar_requested",
                            omc.sidecar_requested, cpp.sidecar_requested)
                          ...
                          bool strip_destination_sidecar)
{
    ...
    return compare_transfer_persist_summary(
...
main(int argc, char** argv)
{
    ...
    if (false) {
        ...
        }
        {
            ...
            transfer_opts.target_suffix = ".tif";
            ok                          = run_transfer_persist_case(
                     "transfer_persist_tiff_exif_embedded_and_sidecar",
                     build_transfer_source_jpeg_exif_xmp_fixture(),
                     build_transfer_target_tiff_fixture("OldTool"),
                     transfer_opts, false)
                 && ok;
        }
        {
            ...
            transfer_opts.target_suffix = ".tif";
            ok                          = run_transfer_persist_case(
                     "transfer_persist_bigtiff_exif_embedded_and_sidecar",
                     build_transfer_source_jpeg_exif_xmp_fixture(),
                     build_transfer_target_bigtiff_fixture("OldTool"),
                     transfer_opts, false)
                 && ok;
        }
        {
            ...
            transfer_opts.target_suffix = ".webp";
            ok                          = run_transfer_persist_case(
                     "transfer_persist_webp_exif_embedded_and_sidecar",
                     build_transfer_source_jpeg_exif_xmp_fixture(),
                     build_transfer_target_webp_fixture("OldTool"),
                     transfer_opts, false)
                 && ok;
        }
        {
            ...
            transfer_opts.target_suffix = ".jp2";
            ok                          = run_transfer_persist_case(
                     "transfer_persist_jp2_exif_embedded_and_sidecar",
                     build_transfer_source_jpeg_exif_xmp_fixture(),
                     build_transfer_target_jp2_fixture("OldTool"),
                     transfer_opts, false)
                 && ok;
        }
        {
            ...
            transfer_opts.target_suffix = ".heic";
            ok                          = run_transfer_persist_case(
                     "transfer_persist_heif_exif_embedded_and_sidecar",
                     build_transfer_source_jpeg_exif_xmp_fixture(),
                     build_transfer_target_heif_fixture("OldTool"),
                     transfer_opts, false)
                 && ok;
        }
        {
            ...
            transfer_opts.target_suffix = ".avif";
            ok                          = run_transfer_persist_case(
                     "transfer_persist_avif_exif_embedded_and_sidecar",
                     build_transfer_source_jpeg_exif_xmp_fixture(),
                     build_transfer_target_avif_fixture("OldTool"),
                     transfer_opts, false)
                 && ok;
        }
        {
            ...
            transfer_opts.target_suffix = ".cr3";
            ok                          = run_transfer_persist_case(
                     "transfer_persist_cr3_exif_embedded_and_sidecar",
                     build_transfer_source_jpeg_exif_xmp_fixture(),
                     build_transfer_target_cr3_fixture("OldTool"),
                     transfer_opts, false)
                 && ok;
    ...
    }
    {
        ...
        transfer_opts.existing_sidecar_precedence
            = OMC_TRANSFER_EXISTING_XMP_PREFER_SOURCE;
        ok = run_transfer_execute_case(
                 "transfer_jpeg_existing_sidecar_sidecar_only_source_wins",
                 build_transfer_source_jpeg_fixture(),
                 build_transfer_target_jpeg_fixture(nullptr, true),
                 transfer_opts)
             && ok;
    ...
    }
    {
        ...
        transfer_opts.carrier_precedence
            = OMC_TRANSFER_EXISTING_XMP_PREFER_EMBEDDED;
        ok = run_transfer_execute_case(
                 "transfer_png_existing_sidecar_and_embedded_embedded_wins",
                 build_transfer_source_jpeg_fixture(),
                 build_transfer_target_png_fixture("Target Embedded Existing"),
                 transfer_opts)
             && ok;
