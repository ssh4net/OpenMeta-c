...
make_casio_type2_makernote(omc_u8* out)
{
    ...
    size = 0U;
    append_bytes(out, &size, "QVC\0", 4U);
...
static omc_size
make_samsung_type2_makernote_u16_picturewizard_malformed(omc_u8* out)
{
    omc_size size;
...
static omc_size
make_pentax_makernote_with_binary_subdirs(omc_u8* out)
{
    omc_size size;
    omc_u32 aeinfo_off;
    omc_u32 shot_off;
    ...
    append_u16le(out, &size, 7U);
    append_u32le(out, &size, 4U);
    append_u32le(out, &size, 0x44332211U);
    ...
    append_u16le(out, &size, 0x0206U);
    append_u16le(out, &size, 7U);
    append_u32le(out, &size, 21U);
    append_u32le(out, &size, aeinfo_off);

    append_u16le(out, &size, 0x0226U);
    append_u16le(out, &size, 7U);
    append_u32le(out, &size, 6U);
    append_u32le(out, &size, shot_off);

    append_u32le(out, &size, 0U);

    for (i = 0U; i < 21U; ++i) {
        append_u8(out, &size, (omc_u8)(i + 1U));
    }
    for (i = 0U; i < 6U; ++i) {
        append_u8(out, &size, (omc_u8)(0xA0U + i));
    }
    return size;
}

static omc_size
make_pentax_type2_makernote(omc_u8* out)
{
    omc_size size;
...
static omc_size
make_pentax_makernote_with_main_0062(omc_u8* out)
{
    omc_size size;
...
static omc_size
make_pentax_makernote_with_zero_face_tables(omc_u8* out)
{
    omc_size size;
    omc_u32 facepos_off;
    omc_u32 facesize_off;
    ...
    append_u16le(out, &size, 0x0227U);
    append_u16le(out, &size, 7U);
    append_u32le(out, &size, 4U);
    append_u32le(out, &size, facepos_off);

    append_u16le(out, &size, 0x0228U);
    append_u16le(out, &size, 7U);
    append_u32le(out, &size, 4U);
    append_u32le(out, &size, facesize_off);

    append_u32le(out, &size, 0U);
    append_u8(out, &size, 0x10U);
    append_u8(out, &size, 0x11U);
    append_u8(out, &size, 0x12U);
    append_u8(out, &size, 0x13U);
    append_u8(out, &size, 0x20U);
    append_u8(out, &size, 0x21U);
    append_u8(out, &size, 0x22U);
    append_u8(out, &size, 0x23U);
    return size;
}

static omc_size
make_ricoh_type2_makernote(omc_u8* out)
{
    omc_size size;
    omc_u32 model_off_pos;
    omc_u32 make_off_pos;
    omc_u32 model_off;
    omc_u32 make_off;

    size = 0U;
    append_text(out, &size, "RICOH");
    append_u8(out, &size, 0U);
    append_u16le(out, &size, 0U);
    ...
    append_u16le(out, &size, 0x0207U);
    append_u16le(out, &size, 2U);
    append_u32le(out, &size, 6U);
    model_off_pos = (omc_u32)size;
    append_u32le(out, &size, 0U);

    append_u16le(out, &size, 0x0300U);
    append_u16le(out, &size, 2U);
    append_u32le(out, &size, 6U);
    make_off_pos = (omc_u32)size;
    append_u32le(out, &size, 0U);

    append_u32le(out, &size, 0U);

    model_off = (omc_u32)size;
    write_u32le_at(out, model_off_pos, model_off);
    append_text(out, &size, "GRIII");
    append_u8(out, &size, 0U);

    make_off = (omc_u32)size;
    write_u32le_at(out, make_off_pos, make_off);
    append_text(out, &size, "RICOH");
    append_u8(out, &size, 0U);
    return size;
}
...
static omc_size
make_ricoh_type2_padded_ifd_makernote(omc_u8* out)
{
    omc_size size;
    omc_u32 model_off_pos;
    omc_u32 make_off_pos;
    ...
    model_off_pos = (omc_u32)size + 8U;
    append_u16le(out, &size, 0x0207U);
    append_u16le(out, &size, 2U);
    append_u32le(out, &size, 5U);
    append_u32le(out, &size, 0U);

    make_off_pos = (omc_u32)size + 8U;
    append_u16le(out, &size, 0x0300U);
    append_u16le(out, &size, 2U);
    append_u32le(out, &size, 6U);
    append_u32le(out, &size, 0U);
...
make_ricoh_native_makernote(omc_u8* out, omc_u32 theta_abs_off)
{
    ...
    append_u16le(out, &size, 7U);
    append_u32le(out, &size, (omc_u32)sizeof(imageinfo_bytes));
    append_u32le(out, &size, imageinfo_off);

    append_u16le(out, &size, 0x1002U);
    append_u16le(out, &size, 4U);
    append_u32le(out, &size, 1U);
    append_u32le(out, &size, 7U);

    append_u16le(out, &size, 0x1003U);
    append_u16le(out, &size, 3U);
    append_u32le(out, &size, 1U);
    append_u16le(out, &size, 2U);
    append_u16le(out, &size, 0U);

    append_u16le(out, &size, 0x2001U);
    append_u16le(out, &size, 7U);
    append_u32le(out, &size, subdir_size);
    append_u32le(out, &size, subdir_off);

    append_u16le(out, &size, 0x4001U);
    append_u16le(out, &size, 4U);
    append_u32le(out, &size, 1U);
    append_u32le(out, &size, theta_abs_off);

    append_u32le(out, &size, 0U);
    append_bytes(out, &size, imageinfo_bytes, sizeof(imageinfo_bytes));

    append_text(out, &size, subdir_hdr);
    append_u8(out, &size, 0U);
    append_u16be(out, &size, 1U);
    append_u16be(out, &size, 0x0007U);
    append_u16be(out, &size, 4U);
    append_u32be(out, &size, 1U);
    append_u32be(out, &size, 5U);
    append_u32be(out, &size, 0U);
    return size;
}

static omc_size
make_ricoh_theta_ifd(omc_u8* out)
{
    omc_size size;

    size = 0U;
    append_u16le(out, &size, 1U);
    append_u16le(out, &size, 0x0003U);
    append_u16le(out, &size, 3U);
    append_u32le(out, &size, 1U);
    append_u16le(out, &size, 9U);
    append_u16le(out, &size, 0U);
...
static omc_size
make_nikon_makernote_with_single_main_bytes_tag(omc_u8* out, omc_u16 tag,
                                                const omc_u8* raw,
                                                omc_size raw_size)
{
    omc_size size;
    ...
    append_u16le(out, &size, tag);
    append_u16le(out, &size, 7U);
    append_u32le(out, &size, (omc_u32)raw_size);
    append_u32le(out, &size, value_off);
    append_u32le(out, &size, 0U);
    append_bytes(out, &size, raw, raw_size);
    return size;
}
...
make_nikon_makernote_with_flashinfo_0108(omc_u8* out)
{
    ...
    append_u16le(out, &size, 0x00A8U);
    append_u16le(out, &size, 7U);
    append_u32le(out, &size, 49U);
    append_u32le(out, &size, flashinfo_off);
    append_u32le(out, &size, 0U);

    append_text(out, &size, "0108");
    for (i = 4U; i < 49U; ++i) {
        append_u8(out, &size, 0U);
    }
    ...
    out[payload_off + 0x0028U] = 0x7FU;
    out[payload_off + 0x0029U] = 0x7EU;
    out[payload_off + 0x002AU] = 0x7DU;
    return size;
}
...
static omc_size
make_test_jpeg_qvci(omc_u8* out)
{
    omc_u8 payload[0x90];
    omc_size size;
...
static omc_size
make_test_bmff_v1_auxl_only(omc_u8* out, omc_u32 major_brand)
{
    omc_u8 pitm_payload[16];
    omc_u8 auxl_payload[24];
    omc_u8 iref_payload[64];
    omc_u8 meta_payload[128];
    omc_u8 ftyp_payload[16];
    omc_size pitm_size;
    ...
    pitm_size = 0U;
    append_fullbox_header(pitm_payload, &pitm_size, 1U);
    append_u32be(pitm_payload, &pitm_size, 0x10001U);

    auxl_size = 0U;
    append_u32be(auxl_payload, &auxl_size, 0x10001U);
    append_u16be(auxl_payload, &auxl_size, 2U);
    append_u32be(auxl_payload, &auxl_size, 0x10002U);
    append_u32be(auxl_payload, &auxl_size, 0x10003U);
                                             ...
                                             omc_u32 major_brand)
{
    ...
    omc_u8 thmb_payload[20];
    omc_u8 cdsc_payload[20];
    omc_u8 iref_payload[128];
    omc_u8 meta_payload[256];
    omc_u8 ftyp_payload[16];
    omc_size pitm_size;
    ...
    dimg_size = 0U;
    append_u32be(dimg_payload, &dimg_size, 0x20002U);
    append_u16be(dimg_payload, &dimg_size, 2U);
    append_u32be(dimg_payload, &dimg_size, 0x30005U);
    append_u32be(dimg_payload, &dimg_size, 0x30006U);

    thmb_size = 0U;
    append_u32be(thmb_payload, &thmb_size, 0x20003U);
    append_u16be(thmb_payload, &thmb_size, 1U);
    append_u32be(thmb_payload, &thmb_size, 0x30007U);

    cdsc_size = 0U;
    append_u32be(cdsc_payload, &cdsc_size, 0x20004U);
    append_u16be(cdsc_payload, &cdsc_size, 1U);
    append_u32be(cdsc_payload, &cdsc_size, 0x30008U);
...
static omc_size
make_test_bmff_duplicate_edges_only(omc_u8* out, omc_u32 major_brand)
{
    omc_u8 pitm_payload[16];
    omc_u8 auxl_a_payload[16];
    omc_u8 auxl_b_payload[16];
    omc_u8 dimg_a_payload[16];
    omc_u8 dimg_b_payload[16];
    omc_u8 thmb_a_payload[16];
    omc_u8 thmb_b_payload[16];
    omc_u8 cdsc_a_payload[16];
    omc_u8 cdsc_b_payload[16];
    omc_u8 iref_payload[256];
    omc_u8 meta_payload[384];
    omc_u8 ftyp_payload[16];
    omc_size pitm_size;
    omc_size auxl_a_size;
    omc_size auxl_b_size;
    omc_size dimg_a_size;
    omc_size dimg_b_size;
    omc_size thmb_a_size;
    omc_size thmb_b_size;
    omc_size cdsc_a_size;
    omc_size cdsc_b_size;
    omc_size iref_size;
    omc_size meta_size;
    omc_size ftyp_size;
    omc_size size;
    ...
    auxl_a_size = 0U;
    append_u16be(auxl_a_payload, &auxl_a_size, 1U);
    append_u16be(auxl_a_payload, &auxl_a_size, 3U);
    append_u16be(auxl_a_payload, &auxl_a_size, 2U);
    append_u16be(auxl_a_payload, &auxl_a_size, 2U);
    append_u16be(auxl_a_payload, &auxl_a_size, 3U);

    auxl_b_size = 0U;
    append_u16be(auxl_b_payload, &auxl_b_size, 1U);
    append_u16be(auxl_b_payload, &auxl_b_size, 1U);
    append_u16be(auxl_b_payload, &auxl_b_size, 3U);

    dimg_a_size = 0U;
    append_u16be(dimg_a_payload, &dimg_a_size, 2U);
    append_u16be(dimg_a_payload, &dimg_a_size, 2U);
    append_u16be(dimg_a_payload, &dimg_a_size, 5U);
    append_u16be(dimg_a_payload, &dimg_a_size, 5U);

    dimg_b_size = 0U;
    append_u16be(dimg_b_payload, &dimg_b_size, 4U);
    append_u16be(dimg_b_payload, &dimg_b_size, 1U);
    append_u16be(dimg_b_payload, &dimg_b_size, 5U);

    thmb_a_size = 0U;
    append_u16be(thmb_a_payload, &thmb_a_size, 3U);
    append_u16be(thmb_a_payload, &thmb_a_size, 2U);
    append_u16be(thmb_a_payload, &thmb_a_size, 7U);
    append_u16be(thmb_a_payload, &thmb_a_size, 7U);

    thmb_b_size = 0U;
    append_u16be(thmb_b_payload, &thmb_b_size, 3U);
    append_u16be(thmb_b_payload, &thmb_b_size, 1U);
    append_u16be(thmb_b_payload, &thmb_b_size, 8U);

    cdsc_a_size = 0U;
    append_u16be(cdsc_a_payload, &cdsc_a_size, 4U);
    append_u16be(cdsc_a_payload, &cdsc_a_size, 3U);
    append_u16be(cdsc_a_payload, &cdsc_a_size, 8U);
    append_u16be(cdsc_a_payload, &cdsc_a_size, 9U);
    append_u16be(cdsc_a_payload, &cdsc_a_size, 9U);

    cdsc_b_size = 0U;
    append_u16be(cdsc_b_payload, &cdsc_b_size, 5U);
    append_u16be(cdsc_b_payload, &cdsc_b_size, 1U);
    append_u16be(cdsc_b_payload, &cdsc_b_size, 8U);
    ...
    append_bmff_box(iref_payload, &iref_size, fourcc('d', 'i', 'm', 'g'),
                    dimg_b_payload, dimg_b_size);
    append_bmff_box(iref_payload, &iref_size, fourcc('t', 'h', 'm', 'b'),
                    thmb_a_payload, thmb_a_size);
    append_bmff_box(iref_payload, &iref_size, fourcc('t', 'h', 'm', 'b'),
                    thmb_b_payload, thmb_b_size);
    append_bmff_box(iref_payload, &iref_size, fourcc('c', 'd', 's', 'c'),
                    cdsc_a_payload, cdsc_a_size);
    append_bmff_box(iref_payload, &iref_size, fourcc('c', 'd', 's', 'c'),
                    cdsc_b_payload, cdsc_b_size);
...
static omc_size
make_test_bmff_item_info_rows_only(omc_u8* out, omc_u32 major_brand)
{
    omc_u8 pitm_payload[16];
    omc_u8 infe_preview[96];
    omc_u8 infe_exif[64];
    omc_u8 iinf_payload[256];
    omc_u8 meta_payload[384];
    omc_u8 ftyp_payload[16];
    omc_size pitm_size;
    omc_size infe_preview_size;
    omc_size infe_exif_size;
    omc_size iinf_size;
    omc_size meta_size;
    omc_size ftyp_size;
    ...
    pitm_size = 0U;
    append_fullbox_header(pitm_payload, &pitm_size, 1U);
    append_u32be(pitm_payload, &pitm_size, 0x10002U);

    infe_preview_size = 0U;
    append_fullbox_header(infe_preview, &infe_preview_size, 3U);
    append_u32be(infe_preview, &infe_preview_size, 0x10001U);
    append_u16be(infe_preview, &infe_preview_size, 0U);
    append_u32be(infe_preview, &infe_preview_size, fourcc('m', 'i', 'm', 'e'));
    append_text(infe_preview, &infe_preview_size, "preview");
    append_u8(infe_preview, &infe_preview_size, 0U);
    append_text(infe_preview, &infe_preview_size, "image/png");
    append_u8(infe_preview, &infe_preview_size, 0U);
    append_text(infe_preview, &infe_preview_size, "gzip");
    append_u8(infe_preview, &infe_preview_size, 0U);

    infe_exif_size = 0U;
    append_fullbox_header(infe_exif, &infe_exif_size, 3U);
    append_u32be(infe_exif, &infe_exif_size, 0x10002U);
    append_u16be(infe_exif, &infe_exif_size, 0U);
    append_u32be(infe_exif, &infe_exif_size, fourcc('E', 'x', 'i', 'f'));
    append_text(infe_exif, &infe_exif_size, "exif");
    append_u8(infe_exif, &infe_exif_size, 0U);
    ...
    iinf_size = 0U;
    append_fullbox_header(iinf_payload, &iinf_size, 2U);
    append_u32be(iinf_payload, &iinf_size, 2U);
    append_bmff_box(iinf_payload, &iinf_size, fourcc('i', 'n', 'f', 'e'),
                    infe_preview, infe_preview_size);
    append_bmff_box(iinf_payload, &iinf_size, fourcc('i', 'n', 'f', 'e'),
                    infe_exif, infe_exif_size);
...
static omc_size
make_test_bmff_primary_mime_item_info_only(omc_u8* out, omc_u32 major_brand)
{
    omc_u8 pitm_payload[16];
    omc_u8 infe_payload[128];
    omc_u8 iinf_payload[192];
    omc_u8 meta_payload[320];
    omc_u8 ftyp_payload[16];
    omc_size pitm_size;
    omc_size infe_size;
    omc_size iinf_size;
    ...
    infe_size = 0U;
    append_fullbox_header(infe_payload, &infe_size, 2U);
    append_u16be(infe_payload, &infe_size, 1U);
    append_u16be(infe_payload, &infe_size, 7U);
    append_u32be(infe_payload, &infe_size, fourcc('m', 'i', 'm', 'e'));
    append_text(infe_payload, &infe_size, "payload");
    append_u8(infe_payload, &infe_size, 0U);
    append_text(infe_payload, &infe_size, "application/rdf+xml");
    append_u8(infe_payload, &infe_size, 0U);
    append_text(infe_payload, &infe_size, "gzip");
    append_u8(infe_payload, &infe_size, 0U);
...
static omc_size
make_test_bmff_item_info_without_pitm_only(omc_u8* out, omc_u32 major_brand)
{
    omc_u8 infe_payload[128];
    omc_u8 iinf_payload[192];
    omc_u8 meta_payload[256];
    ...
    infe_size = 0U;
    append_fullbox_header(infe_payload, &infe_size, 2U);
    append_u16be(infe_payload, &infe_size, 3U);
    append_u16be(infe_payload, &infe_size, 0U);
    append_u32be(infe_payload, &infe_size, fourcc('m', 'i', 'm', 'e'));
    append_text(infe_payload, &infe_size, "sidecar");
    append_u8(infe_payload, &infe_size, 0U);
    append_text(infe_payload, &infe_size, "application/json");
    append_u8(infe_payload, &infe_size, 0U);
    append_u8(infe_payload, &infe_size, 0U);
...
static omc_size
make_test_bmff_primary_uri_item_info_only(omc_u8* out, omc_u32 major_brand)
{
    omc_u8 pitm_payload[16];
    omc_u8 infe_payload[128];
    omc_u8 iinf_payload[192];
    ...
    append_u16be(infe_payload, &infe_size, 1U);
    append_u16be(infe_payload, &infe_size, 3U);
    append_u32be(infe_payload, &infe_size, fourcc('u', 'r', 'i', ' '));
    append_text(infe_payload, &infe_size, "link");
    append_u8(infe_payload, &infe_size, 0U);
    append_text(infe_payload, &infe_size, "https://ns.example/item");
    append_u8(infe_payload, &infe_size, 0U);
...
static void
test_read_jpeg_casio_qvci(void)
{
    omc_u8 jpeg[256];
    omc_size jpeg_size;
    omc_store store;
    ...
    jpeg_size = make_test_jpeg_qvci(jpeg);
    omc_store_init(&store);
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    assert(res.exif.entries_decoded == 6U);
    assert(store.block_count == 1U);

    entry = find_exif_entry_typed(&store, "mk_casio_qvci_0", 0x002CU,
                                  OMC_VAL_SCALAR, OMC_ELEM_U8);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 3U);
    block = omc_store_block(&store, entry->origin.block);
    assert(block != (const omc_block_info*)0);
    assert(block->format == OMC_SCAN_FMT_JPEG);
    assert(block->kind == OMC_BLK_MAKERNOTE);

    entry = find_exif_entry(&store, "mk_casio_qvci_0", 0x004DU);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_TEXT);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size == 19U);
    assert(memcmp(view.data, "2025:01:02 03:04:05", 19U) == 0);

    entry = find_exif_entry(&store, "mk_casio_qvci_0", 0x0062U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_TEXT);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size == 6U);
    assert(memcmp(view.data, "QV7000", 6U) == 0);
...
static void
test_read_tiff_casio_simple_meta(void)
{
    omc_u8 makernote[96];
    omc_u8 tiff[256];
    omc_size makernote_size;
    ...
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size >= 4U);
    assert(view.data[0] == 0x40U);
    assert(view.data[1] == 0x01U);
    assert(view.data[2] == 0xF0U);
    assert(view.data[3] == 0x00U);
...
test_read_tiff_samsung_makernote(void)
{
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_samsung_type2_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_BYTES);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size == 4U);
    assert(view.data[0] == 0x01U);
    assert(view.data[1] == 0x00U);
    assert(view.data[2] == 0x21U);
    assert(view.data[3] == 0x00U);
    entry = find_exif_entry(&store, "mk_samsung_type2_0", 0x0021U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_ARRAY);
    assert(entry->value.elem_type == OMC_ELEM_U16);
    ...
    omc_store_reset(&store);
    makernote_size = make_samsung_type2_makernote_u16_picturewizard_malformed(
        makernote);
    tiff_size = make_test_tiff_with_make_and_makernote_count(
        tiff, "SAMSUNG", makernote, makernote_size, (omc_u32)makernote_size);
    res = omc_read_simple(tiff, tiff_size, &store, blocks, 4U, ifds, 8U,
    ...
    entry = find_exif_entry(&store, "mk_samsung_type2_0", 0x0021U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_EMPTY);
    assert(entry->value.elem_type == OMC_ELEM_U8);
    assert(entry->value.count == 0U);
    assert((entry->flags & OMC_ENTRY_FLAG_UNREADABLE) != 0U);
...
test_read_tiff_pentax_makernote(void)
{
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_pentax0", 0x0001U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U16);
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_pentax_srinfo_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 0x11U);
    entry = find_exif_entry(&store, "mk_pentax_aeinfo2_0", 0x0014U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 21U);
    entry = find_exif_entry(&store, "mk_pentax_shotinfo_0", 0x0005U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 0xA5U);
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    assert(find_exif_entry(&store, "mk_pentax0", 0x0001U)
           == (const omc_entry*)0);
    entry = find_exif_entry(&store, "mk_pentax_type2_0", 0x1000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U16);
    assert(entry->value.u.u64 == 0x1234U);
...
static void
test_read_tiff_pentax_contextual_name_and_zero_faces(void)
{
    omc_u8 makernote[256];
    omc_u8 tiff[512];
    omc_size makernote_size;
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_pentax0", 0x0062U);
    assert(entry != (const omc_entry*)0);
    assert((entry->flags & OMC_ENTRY_FLAG_CONTEXTUAL_NAME) != 0U);
    assert(entry->origin.name_context_kind
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    assert(find_exif_entry(&store, "mk_pentax0", 0x0227U)
           != (const omc_entry*)0);
    assert(find_exif_entry(&store, "mk_pentax0", 0x0228U)
           != (const omc_entry*)0);
    assert(find_exif_entry(&store, "mk_pentax_facepos_0", 0x0000U)
           == (const omc_entry*)0);
    assert(find_exif_entry(&store, "mk_pentax_facesize_0", 0x0000U)
           == (const omc_entry*)0);
...
static void
test_read_tiff_ricoh_makernote(void)
{
    omc_u8 makernote[128];
    omc_u8 tiff[256];
    omc_size makernote_size;
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_ricoh_type2_0", 0x0207U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_TEXT);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size == 5U);
    assert(memcmp(view.data, "GRIII", 5U) == 0);
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_ricoh_type2_0", 0x0104U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U32);
    ...
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size == 4U);
    assert(memcmp(view.data, "HZ15", 4U) == 0);

    omc_store_fini(&store);
}

static void
test_read_tiff_ricoh_native_makernote(void)
{
    omc_u8 makernote[256];
    omc_u8 theta_ifd[32];
    omc_u8 tiff[512];
    ...
    append_bytes(tiff, &tiff_size, theta_ifd, theta_size);
    ...
    entry = find_exif_entry(&store, "mk_ricoh0", 0x1002U);
    assert(entry != (const omc_entry*)0);
    assert((entry->flags & OMC_ENTRY_FLAG_CONTEXTUAL_NAME) != 0U);
    assert(entry->origin.name_context_kind
    ...
    entry = find_exif_entry(&store, "mk_ricoh0", 0x1003U);
    assert(entry != (const omc_entry*)0);
    assert((entry->flags & OMC_ENTRY_FLAG_CONTEXTUAL_NAME) != 0U);
    assert(entry->origin.name_context_kind
           == OMC_ENTRY_NAME_CTX_RICOH_MAIN_COMPAT);
    ...
    entry = find_exif_entry(&store, "mk_ricoh_imageinfo_0", 0x0003U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U8);
    assert(entry->value.u.u64 == 0U);

    entry = find_exif_entry(&store, "mk_ricoh_subdir_0", 0x0007U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U32);
    assert(entry->value.u.u64 == 5U);

    entry = find_exif_entry(&store, "mk_ricoh_thetasubdir_0", 0x0003U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U16);
...
test_read_tiff_nikon_preview_and_aftune_makernote(void)
{
    ...
    assert(find_exif_entry(&store, "mk_nikon_preview_0", 0x0103U)
           == (const omc_entry*)0);
    assert(find_exif_entry(&store, "mk_nikon_preview_0", 0x0202U)
           == (const omc_entry*)0);
...
static void
test_read_tiff_nikon_lensdata_and_colorbalance_public_surface(void)
{
    omc_u8 makernote[512];
    omc_u8 tiff[1024];
    omc_size makernote_size;
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_nikon_lensdata0100_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    entry = find_exif_entry(&store, "mk_nikon_lensdata0100_0", 0x000AU);
    assert(entry != (const omc_entry*)0);
    entry = find_exif_entry(&store, "mk_nikon_lensdata0100_0", 0x000CU);
    assert(entry != (const omc_entry*)0);
    entry = find_exif_entry(&store, "mk_nikon_lensdata0100_0", 0x0006U);
    assert(entry != (const omc_entry*)0);
    entry = find_exif_entry(&store, "mk_nikon_lensdata0100_0", 0x000BU);
    assert(entry != (const omc_entry*)0);
    omc_store_fini(&store);
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_nikon_lensdata0101_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
...
static void
test_read_tiff_nikon_flashinfo_and_main_contexts(void)
{
    omc_u8 makernote[256];
    omc_u8 tiff[512];
    omc_size makernote_size;
...
test_read_tiff_canon_camera_info_and_colordata_makernote(void)
{
    ...
    entry = find_exif_entry(&store, "mk_canon_camerainfo_0", 0x0189U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U16);
    assert(entry->value.u.u64 == 0U);

    entry = find_exif_entry(&store, "mk_canon_camerainfo_0", 0x01ACU);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_TEXT);
...
static void
test_read_bmff_v1_auxl_edges(void)
{
    omc_u8 file_bytes[512];
    omc_size file_size;
    omc_store store;
    ...
    file_size = make_test_bmff_v1_auxl_only(file_bytes,
                                            fourcc('h', 'e', 'i', 'c'));
    omc_store_init(&store);
...
static void
test_read_bmff_v1_nonprimary_typed_edges(void)
{
    omc_u8 file_bytes[512];
    omc_size file_size;
    omc_store store;
...
static void
test_read_bmff_duplicate_edges(void)
{
    ...
    file_size = make_test_bmff_duplicate_edges_only(file_bytes,
                                                    fourcc('h', 'e', 'i', 'c'));
    omc_store_init(&store);
    ...
    assert(count_bmff_field_scalar_value(&store, "iref.edge_count", 14U) == 1U);
    ...
    assert(count_bmff_field_scalar_value(&store, "iref.auxl.item_count", 3U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.auxl.item_id", 1U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.auxl.item_id", 2U)
           == 1U);
    ...
    assert(count_bmff_field_scalar_value(&store, "iref.dimg.item_count", 3U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.dimg.item_id", 2U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.dimg.item_id", 4U)
           == 1U);
    ...
    assert(count_bmff_field_scalar_value(&store, "iref.thmb.item_count", 3U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.thmb.item_id", 3U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.thmb.item_id", 7U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.thmb.item_id", 8U)
           == 1U);

    assert(count_bmff_field_scalar_value(&store, "iref.cdsc.item_count", 4U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.cdsc.item_id", 4U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.cdsc.item_id", 5U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.cdsc.item_id", 8U)
           == 1U);
    assert(count_bmff_field_scalar_value(&store, "iref.cdsc.item_id", 9U)
           == 1U);
...
static void
test_read_bmff_item_info_rows(void)
{
    omc_u8 file_bytes[512];
    omc_size file_size;
    omc_store store;
    ...
    file_size = make_test_bmff_item_info_rows_only(file_bytes,
                                                   fourcc('h', 'e', 'i', 'c'));
    omc_store_init(&store);
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.bmff.status == OMC_BMFF_OK);
    assert(count_bmff_field_scalar_value(&store, "item.info_count", 2U) == 1U);
    ...
    assert(find_bmff_field_text(&store, "item.name", "exif")
           != (const omc_entry*)0);
    assert(find_bmff_field_text(&store, "item.content_type", "image/png")
           != (const omc_entry*)0);
...
static void
test_read_bmff_primary_mime_item_info(void)
{
    omc_u8 file_bytes[512];
    omc_size file_size;
    omc_store store;
    ...
    assert(find_bmff_field_text(&store, "primary.content_type",
                                "application/rdf+xml")
           != (const omc_entry*)0);
    assert(find_bmff_field_text(&store, "primary.content_encoding", "gzip")
           != (const omc_entry*)0);
...
static void
test_read_bmff_item_info_without_pitm(void)
{
    omc_u8 file_bytes[512];
    omc_size file_size;
    omc_store store;
    ...
    assert(find_bmff_field_text(&store, "item.content_type", "application/json")
           != (const omc_entry*)0);
    assert(count_bmff_field(&store, "meta.primary_item_id") == 0U);
    assert(count_bmff_field(&store, "primary.item_type") == 0U);
    assert(count_bmff_field(&store, "primary.item_name") == 0U);

    omc_store_fini(&store);
}

static void
test_read_bmff_primary_uri_item_info(void)
{
    omc_u8 file_bytes[512];
    omc_size file_size;
    omc_store store;
...
test_read_tiff_kodak_makernotes(void)
{
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_kodak_type2_0", 0x0028U);
    assert(entry != (const omc_entry*)0);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_kodak_type7_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry_typed(&store, "mk_kodak_type5_0", 0x0014U,
                                  OMC_VAL_SCALAR, OMC_ELEM_U32);
    assert(entry != (const omc_entry*)0);
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry_typed(&store, "mk_kodak_type8_0", 0x0200U,
                                  OMC_VAL_SCALAR, OMC_ELEM_U16);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 1U);
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry_typed(&store, "mk_kodak_type11_0", 0x0200U,
                                  OMC_VAL_SCALAR, OMC_ELEM_U16);
    assert(entry != (const omc_entry*)0);
...
test_read_tiff_sigma_makernotes(void)
{
    ...
    assert(res.scan.status == OMC_SCAN_OK);
    assert(res.exif.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_sigma0", 0x0120U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_ARRAY);
    ...
    entry = find_exif_entry(&store, "mk_sigma0", 0x0121U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_ARRAY);
    assert(entry->value.elem_type == OMC_ELEM_F32_BITS);
    assert(entry->value.count == 30U);
    ...
    entry = find_exif_entry(&store, "mk_sigma_wbsettings_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_ARRAY);
    assert(entry->value.elem_type == OMC_ELEM_F32_BITS);
    assert(entry->value.count == 3U);
    entry = find_exif_entry(&store, "mk_sigma_wbsettings2_0", 0x001BU);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_ARRAY);
    assert(entry->value.elem_type == OMC_ELEM_F32_BITS);
    assert(entry->value.count == 3U);
...
main(void)
{
    ...
    test_read_tiff_nikon_lensdata_and_colorbalance_public_surface();
    test_read_tiff_nikon_flashinfo_and_main_contexts();
    ...
    test_read_bmff_v1_nonprimary_typed_edges();
    test_read_bmff_duplicate_edges();
    test_read_bmff_item_info_rows();
    test_read_bmff_primary_mime_item_info();
    test_read_bmff_item_info_without_pitm();
    test_read_bmff_primary_uri_item_info();
