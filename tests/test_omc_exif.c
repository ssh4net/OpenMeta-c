...
make_samsung_stmn_makernote(omc_u8* out)
{
    ...
    size = 0U;
    append_raw(out, &size, "STMN100", 7U);
    append_u8(out, &size, 0U);
...
make_ricoh_native_makernote(omc_u8* out, omc_u32 theta_abs_off)
{
    ...
    append_u32le(out, &size, 0U);
    append_raw(out, &size, imageinfo_bytes, sizeof(imageinfo_bytes));

    append_bytes(out, &size, subdir_hdr);
    append_u8(out, &size, 0U);
    append_u16be(out, &size, 1U);
                                      ...
                                      int truncate_next_ifd)
{
    ...
    append_raw(out, &size, facedet_raw, sizeof(facedet_raw));
    append_raw(out, &size, facerec_raw, sizeof(facerec_raw));
                                               ...
                                               int truncate_next_ifd)
{
    ...
    append_raw(out, &size, facedet, sizeof(facedet));
    append_raw(out, &size, facerec, sizeof(facerec));
    append_raw(out, &size, timeinfo, sizeof(timeinfo));
    return size;
}
...
make_olympus_makernote_omsystem_nested_subifds(omc_u8* out)
{
    ...
    size = 0U;
    append_bytes(out, &size, "OM SYSTEM");
    append_u8(out, &size, 0U);
    append_u8(out, &size, 0U);
    append_u8(out, &size, 0U);
    append_bytes(out, &size, "II");
...
static omc_size
make_sony_makernote_tag940e_ciphered(omc_u8* out)
{
    omc_u8 plain[0x1A14];
    omc_u32 i;
    ...
    plain[0x1A07U] = 3U;
    for (i = 0U; i < 12U; ++i) {
        plain[0x1A08U + i] = (omc_u8)(0xA0U + i);
    }
    return make_sony_makernote_ciphered_blob(out, 0x940EU, plain,
                                             sizeof(plain));
}

static omc_size
make_sony_makernote_tag940e_afinfo_ciphered(omc_u8* out)
{
    omc_u8 plain[0x0180];
    omc_u32 i;
    ...
    plain[0x000BU] = 10U;
    for (i = 0U; i < 30U; ++i) {
        write_u16le_at(plain, 0x0011U + (i * 2U), (omc_u16)(100U + i));
    }
    plain[0x016EU] = 0x78U;
    plain[0x016FU] = 0x56U;
    plain[0x0170U] = 0x34U;
...
static void
test_samsung_stmn_makernote(void)
{
    omc_u8 makernote[96];
    omc_u8 tiff[256];
    omc_size makernote_size;
    ...
    entry = find_exif_entry(&store, "mk_samsung0", 0x0002U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U32);
    assert(entry->value.u.u64 == 0x12345678U);

    entry = find_exif_entry(&store, "mk_samsung0", 0x0003U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U32);
    assert(entry->value.u.u64 == 0x00010002U);
...
static void
test_samsung_type2_picturewizard(void)
{
    omc_u8 makernote[96];
    omc_u8 tiff[256];
    omc_size makernote_size;
    ...
    entry = find_exif_entry(&store, "mk_samsung_type2_0", 0x0004U);
    assert(entry != (const omc_entry*)0);
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_CANONICAL,
                           "Samsung_Type2_0x0004");
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_EXIFTOOL_COMPAT,
                           "Samsung_Type2_0x0004");
...
static void
test_samsung_type2_picturewizard_u16_array(void)
{
    omc_u8 makernote[64];
    omc_u8 tiff[192];
    omc_size makernote_size;
    ...
    entry = find_exif_entry(&store, "mk_samsung_picturewizard_0", 0x0004U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 55U);

    omc_store_fini(&store);
}

static void
test_samsung_compat_tag0_and_a002_names(void)
{
    omc_u8 makernote[64];
    omc_u8 tiff[192];
    omc_size makernote_size;
    ...
    entry = find_exif_entry(&store, "mk_samsung_type2_0", 0xA003U);
    assert(entry != (const omc_entry*)0);
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_CANONICAL,
                           "LensType");
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_EXIFTOOL_COMPAT,
...
static void
test_minolta_makernote_and_binary_subdirs(void)
{
    omc_u8 makernote[128];
    omc_u8 tiff[256];
    omc_size makernote_size;
...
static void
test_motorola_makernote_context(void)
{
    omc_u8 makernote[64];
    omc_u8 tiff[256];
    omc_size makernote_size;
...
static void
test_pentax_makernote_root_and_binary_subdirs(void)
{
    ...
    entry = find_exif_entry(&store, "mk_pentax_srinfo_0", 0x0003U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.u.u64 == 0x44U);

    entry = find_exif_entry(&store, "mk_pentax_faceinfo_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.u.u64 == 3U);

    entry = find_exif_entry(&store, "mk_pentax_aeinfo2_0", 0x0001U);
    assert(entry != (const omc_entry*)0);
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_CANONICAL,
                           "Pentax_0x0001");
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_EXIFTOOL_COMPAT,
    ...
    entry = find_exif_entry(&store, "mk_pentax_aeinfo2_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.u.u64 == 1U);
    ...
    entry = find_exif_entry(&store, "mk_pentax_shotinfo_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.u.u64 == 0xA0U);
...
static void
test_pentax_makernote_type2_routing(void)
{
    omc_u8 makernote[256];
    omc_u8 tiff[512];
    omc_size makernote_size;
    ...
    assert(find_exif_entry(&store, "mk_pentax0", 0x0001U)
           == (const omc_entry*)0);
    entry = find_exif_entry(&store, "mk_pentax_type2_0", 0x0001U);
    assert(entry != (const omc_entry*)0);
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_CANONICAL,
                           "RecordingMode");
    omc_store_fini(&store);
...
static void
test_pentax_contextual_name_and_zero_face_tables(void)
{
    omc_u8 makernote[256];
    omc_u8 tiff[512];
    omc_size makernote_size;
...
static void
test_ricoh_type2_makernote(void)
{
    omc_u8 makernote[128];
    omc_u8 tiff[256];
    omc_size makernote_size;
    ...
    entry = find_exif_entry(&store, "mk_ricoh_type2_0", 0x0300U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_TEXT);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size == 5U);
    assert(memcmp(view.data, "RICOH", 5U) == 0);

    omc_store_fini(&store);
}

static void
test_ricoh_padded_type2_makernote(void)
{
    omc_u8 makernote[128];
    omc_u8 tiff[256];
    omc_size makernote_size;
...
static void
test_ricoh_native_makernote_and_subdirs(void)
{
    omc_u8 makernote[256];
    omc_u8 theta_ifd[32];
    omc_u8 tiff[512];
    ...
    append_raw(tiff, &tiff_size, theta_ifd, theta_size);
    ...
    assert(entry->value.u.u64 == 7U);
    assert((entry->flags & OMC_ENTRY_FLAG_CONTEXTUAL_NAME) != 0U);
    assert(entry->origin.name_context_kind == OMC_ENTRY_NAME_CTX_RICOH_MAIN_COMPAT);
    assert(entry->origin.name_context_variant == 1U);
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_CANONICAL,
                           "DriveMode");
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_EXIFTOOL_COMPAT,
    ...
    entry = find_exif_entry(&store, "mk_ricoh_imageinfo_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U8);
    assert(entry->value.u.u64 == 74U);
...
static void
test_panasonic_makernote_subdirs(void)
{
...
static void
test_panasonic_makernote_truncated_next_ifd(void)
{
    omc_u8 makernote[160];
    omc_u8 tiff[256];
    omc_size makernote_size;
...
static void
test_panasonic_type2_makernote(void)
{
    omc_u8 makernote[32];
    omc_u8 tiff[96];
    omc_size makernote_size;
...
static void
test_panasonic_extended_makernote_subdirs(void)
{
    ...
    entry = find_exif_entry(&store, "mk_panasonic_facerecinfo_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 3U);
    ...
    entry = find_exif_entry(&store, "mk_panasonic_facerecinfo_0", 0x0048U);
    assert(entry != (const omc_entry*)0);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size >= 8U);
    assert(view.data[0] == 5U);
    assert(view.data[6] == 8U);
    ...
    entry = find_exif_entry(&store, "mk_panasonic_facerecinfo_0", 0x0078U);
    assert(entry != (const omc_entry*)0);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size >= 8U);
    assert(view.data[0] == 9U);
    assert(view.data[6] == 12U);

    entry = find_exif_entry(&store, "mk_panasonic_facerecinfo_0", 0x0080U);
    assert(entry != (const omc_entry*)0);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size == 2U);
    assert(memcmp(view.data, "19", 2U) == 0);
...
static void
test_panasonic_extended_truncated_next_ifd(void)
{
    omc_u8 makernote[320];
    omc_u8 tiff[512];
    omc_size makernote_size;
...
static void
test_olympus_signature_makernote(void)
{
    omc_u8 makernote[64];
    omc_u8 tiff[160];
    omc_size makernote_size;
...
static void
test_olympus_omsystem_nested_subifds(void)
{
    omc_u8 makernote[256];
    omc_u8 tiff[384];
    omc_size makernote_size;
...
static void
test_olympus_oldstyle_nested_subifds(void)
{
    omc_u8 makernote[160];
    omc_u8 tiff[320];
    omc_size makernote_size;
...
static void
test_olympus_omsystem_main_subifd_matrix(void)
{
    omc_u8 makernote[320];
    omc_u8 tiff[384];
    omc_size makernote_size;
    ...
    entry = find_exif_entry(&store, "mk_olympus_imageprocessing_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 5U);
...
static void
test_olympus_focusinfo_name_context(void)
{
    omc_u8 makernote[160];
    omc_u8 tiff[256];
    omc_size makernote_size;
...
static void
test_sony_tag940e_variants(void)
{
    omc_u8 makernote[8192];
    omc_u8 tiff[16384];
    omc_size makernote_size;
    ...
    makernote_size = make_sony_makernote_tag940e_ciphered(makernote);
    tiff_size = make_test_tiff_with_make_model_and_makernote_count(
    ...
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_sony_tag940e_0", 0x1A06U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U8);
    assert(entry->value.u.u64 == 2U);
    entry = find_exif_entry(&store, "mk_sony_tag940e_0", 0x1A08U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_BYTES);
    assert(entry->value.elem_type == OMC_ELEM_U8);
    assert(entry->value.count == 12U);
    omc_store_fini(&store);

    makernote_size = make_sony_makernote_tag940e_afinfo_ciphered(makernote);
    tiff_size = make_test_tiff_with_make_model_and_makernote_count(
        tiff, "Sony", "SLT-A99", makernote, makernote_size,
        (omc_u32)makernote_size);
    assert(patch_sony_makernote_value_offset_in_tiff(tiff, tiff_size));
    ...
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_sony_afinfo_0", 0x0002U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U8);
    assert(entry->value.u.u64 == 2U);
    entry = find_exif_entry(&store, "mk_sony_afinfo_0", 0x016EU);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_U32);
    assert(entry->value.u.u64 == 0x12345678U);
    entry = find_exif_entry(&store, "mk_sony_afstatus19_0", 0x0000U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_I16);
    assert(entry->value.u.i64 == 100);
    entry = find_exif_entry(&store, "mk_sony_afstatus19_0", 0x0004U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_SCALAR);
    assert(entry->value.elem_type == OMC_ELEM_I16);
    assert(entry->value.u.i64 == 102);
    omc_store_fini(&store);
}
...
static omc_size
make_apple_makernote(omc_u8* out)
{
    omc_size size;

    size = 0U;
    append_bytes(out, &size, "Apple iOS");
    append_u8(out, &size, 0U);
    append_u8(out, &size, 0U);
    append_u8(out, &size, 1U);
    append_bytes(out, &size, "MM");
...
make_apple_makernote_extended(omc_u8* out)
{
    ...
    append_u16be(out, &size, 2U);
    append_u16be(out, &size, 3U);
    append_bytes(out, &size, "HELLO");
    append_u8(out, &size, 0U);
    return size;
}

static omc_size
make_flir_makernote(omc_u8* out)
{
    omc_size size;
...
static omc_size
make_sigma_makernote_main_u16(omc_u8* out, omc_u16 tag, omc_u16 value)
{
    omc_size size;
...
static omc_size
make_sigma_wb_makernote(omc_u8* out)
{
    omc_size size;
    omc_u32 data1_off;
    omc_u32 data2_off;
    ...
    append_u16le(out, &size, 0x0120U);
    append_u16le(out, &size, 11U);
    append_u32le(out, &size, 30U);
    data1_off = 30U;
    append_u32le(out, &size, data1_off);

    append_u16le(out, &size, 0x0121U);
    append_u16le(out, &size, 11U);
    append_u32le(out, &size, 30U);
    data2_off = data1_off + 120U;
    append_u32le(out, &size, data2_off);

    append_u32le(out, &size, 0U);

    for (i = 0U; i < 30U; ++i) {
        append_u32le(out, &size, f32_bits((float)(i + 1U)));
    }
    for (i = 0U; i < 30U; ++i) {
        append_u32le(out, &size, f32_bits((float)(i + 41U)));
    }
...
static omc_size
make_reconyx_h2_makernote(omc_u8* out)
{
    memset(out, 0, 0x009CU);
    memcpy(out, "RECONYXH2", 9U);
    write_u16le_at(out, 0x0010U, 7U);
    write_u16le_at(out, 0x0012U, 9U);
    write_u16le_at(out, 0x0014U, 11U);
    write_u16le_at(out, 0x0016U, 22U);
    write_u16le_at(out, 0x003EU, 2025U);
    write_u16le_at(out, 0x0040U, 3U);
    write_u16le_at(out, 0x0042U, 16U);
    write_u16le_at(out, 0x0044U, 12U);
    write_u16le_at(out, 0x0046U, 34U);
    write_u16le_at(out, 0x0048U, 56U);
    out[0x0034U] = (omc_u8)'M';
    out[0x0035U] = (omc_u8)'D';
    memcpy(out + 0x0068U, "NIGHT_RUN", 9U);
    return 0x009CU;
}

static void
test_small_vendor_makernotes(void)
{
    omc_u8 tiff[2048];
    omc_u8 makernote[512];
    omc_size tiff_size;
    ...
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_apple0", 0x0007U);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.kind == OMC_VAL_ARRAY);
    assert(entry->value.elem_type == OMC_ELEM_U16);
    assert(entry->value.count == 3U);
    ...
    tiff_size = make_test_tiff_with_make_and_makernote_count(
        tiff, "FLIR", makernote, makernote_size, (omc_u32)makernote_size);
    omc_store_init(&store);
    omc_exif_opts_init(&opts);
    opts.decode_makernote = 1;
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry_typed(&store, "mk_flir0", 0x0001U, OMC_VAL_SCALAR,
                                  OMC_ELEM_U32);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 99U);
    omc_store_fini(&store);
    ...
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry_typed(&store, "mk_nintendo_camerainfo_0", 0x0008U,
                                  OMC_VAL_SCALAR, OMC_ELEM_U32);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 0x12345678U);
    entry = find_exif_entry_typed(&store, "mk_nintendo_camerainfo_0", 0x0028U,
                                  OMC_VAL_SCALAR, OMC_ELEM_F32_BITS);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.f32_bits == 0x3FC00000U);
    omc_store_fini(&store);
    ...
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry_typed(&store, "mk_hp_type6_0", 0x000CU,
                                  OMC_VAL_SCALAR, OMC_ELEM_URATIONAL);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.ur.numer == 28U);
    assert(entry->value.u.ur.denom == 10U);
    entry = find_exif_entry(&store, "mk_hp_type6_0", 0x0058U);
    assert(entry != (const omc_entry*)0);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size == 8U);
    assert(memcmp(view.data, "HP-12345", 8U) == 0);
    omc_store_fini(&store);
    ...
    entry = find_exif_entry_typed(&store, "mk_kodak0", 0x000CU, OMC_VAL_SCALAR,
                                  OMC_ELEM_U16);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 800U);
    entry = find_exif_entry(&store, "mk_kodak0", 0x0014U);
    assert(entry != (const omc_entry*)0);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size == 11U);
    assert(memcmp(view.data, "03:04:05.06", 11U) == 0);
    omc_store_fini(&store);

    makernote_size = make_reconyx_h2_makernote(makernote);
    tiff_size = make_test_tiff_with_make_and_makernote_count(
        tiff, "RECONYX", makernote, makernote_size, (omc_u32)makernote_size);
    omc_store_init(&store);
    omc_exif_opts_init(&opts);
    opts.decode_makernote = 1;
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry_typed(&store, "mk_reconyx_hyperfire2_0", 0x0010U,
                                  OMC_VAL_SCALAR, OMC_ELEM_U16);
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 7U);
    entry = find_exif_entry(&store, "mk_reconyx_hyperfire2_0", 0x0068U);
    assert(entry != (const omc_entry*)0);
    view = omc_arena_view(&store.arena, entry->value.u.ref);
    assert(view.size == 9U);
    assert(memcmp(view.data, "NIGHT_RUN", 9U) == 0);
    omc_store_fini(&store);
}

static void
test_kodak_makernote_variants(void)
{
    omc_u8 tiff[2048];
    omc_u8 makernote[256];
    omc_size tiff_size;
    ...
    assert(memcmp(view.data, "C33000641478", 12U) == 0);
    ...
    omc_store_reset(&store);
    tiff_size = make_test_tiff_with_make_model_and_makernote_count(
        tiff, "KODAK", "PIXPRO AZ251", makernote, makernote_size,
        (omc_u32)makernote_size);
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    ...
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 1U);
    assert(find_exif_entry(&store, "mk_kodak_camerainfo_0", 0x0200U)
           == (const omc_entry*)0);
    ...
    assert(entry != (const omc_entry*)0);
    assert(entry->value.u.u64 == 1U);
    assert(find_exif_entry(&store, "mk_kodak_type8_0", 0x0200U)
           == (const omc_entry*)0);
...
static void
test_sigma_makernote_variants(void)
{
    omc_u8 tiff[2048];
    omc_u8 makernote[512];
    omc_size tiff_size;
    ...
    makernote_size = make_sigma_makernote_main_u16(makernote, 0x0033U, 41U);
    tiff_size = make_test_tiff_with_make_model_and_makernote_count(
    ...
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_sigma0", 0x0033U);
    assert(entry != (const omc_entry*)0);
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_CANONICAL,
                           "ExposureTime2");
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_EXIFTOOL_COMPAT,
    ...
    omc_store_reset(&store);
    makernote_size = make_sigma_makernote_main_u16(makernote, 0x0026U, 41U);
    tiff_size = make_test_tiff_with_make_model_and_makernote_count(
        tiff, "SIGMA", "SIGMA fp L", makernote, makernote_size,
        (omc_u32)makernote_size);
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_sigma0", 0x0026U);
    assert(entry != (const omc_entry*)0);
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_CANONICAL,
                           "FileFormat");
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_EXIFTOOL_COMPAT,
    ...
    omc_store_reset(&store);
    makernote_size = make_sigma_makernote_main_u16(makernote, 0x0034U, 41U);
    tiff_size = make_test_tiff_with_make_model_and_makernote_count(
    ...
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_sigma0", 0x0034U);
    assert(entry != (const omc_entry*)0);
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_CANONICAL,
                           "BurstShot");
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_EXIFTOOL_COMPAT,
    ...
    omc_store_reset(&store);
    makernote_size = make_sigma_makernote_main_u16(makernote, 0x004BU, 41U);
    tiff_size = make_test_tiff_with_make_model_and_makernote_count(
        tiff, "SIGMA", "sd Quattro H", makernote, makernote_size,
        (omc_u32)makernote_size);
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
                       &opts);
    assert(res.status == OMC_EXIF_OK);
    entry = find_exif_entry(&store, "mk_sigma0", 0x004BU);
    assert(entry != (const omc_entry*)0);
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_CANONICAL,
                           "ExposureTime2");
    assert_exif_entry_name(&store, entry, OMC_EXIF_NAME_EXIFTOOL_COMPAT,
    ...
    omc_store_reset(&store);
    makernote_size = make_sigma_wb_makernote(makernote);
    tiff_size = make_test_tiff_with_make_model_and_makernote_count(
        tiff, "SIGMA", "SIGMA fp L", makernote, makernote_size,
        (omc_u32)makernote_size);
    res = omc_exif_dec(tiff, tiff_size, &store, OMC_INVALID_BLOCK_ID, ifds, 8U,
    ...
    memcpy(&bits, view.data + 8U, sizeof(bits));
    assert(bits == f32_bits(3.0f));
...
main(void)
{
    ...
    test_samsung_stmn_makernote();
    test_samsung_type2_picturewizard();
    test_samsung_type2_picturewizard_u16_array();
    test_samsung_compat_tag0_and_a002_names();
    test_minolta_makernote_and_binary_subdirs();
    test_motorola_makernote_context();
    test_pentax_makernote_root_and_binary_subdirs();
    test_pentax_makernote_type2_routing();
    test_pentax_contextual_name_and_zero_face_tables();
    test_ricoh_type2_makernote();
    test_ricoh_padded_type2_makernote();
    test_ricoh_native_makernote_and_subdirs();
    test_panasonic_makernote_subdirs();
    test_panasonic_makernote_truncated_next_ifd();
    test_panasonic_type2_makernote();
    test_panasonic_extended_makernote_subdirs();
    test_panasonic_extended_truncated_next_ifd();
    test_olympus_signature_makernote();
    test_olympus_omsystem_nested_subifds();
    test_olympus_oldstyle_nested_subifds();
    test_olympus_omsystem_main_subifd_matrix();
    test_olympus_focusinfo_name_context();
    ...
    test_small_vendor_makernotes();
    test_kodak_makernote_variants();
    test_sigma_makernote_variants();
    return 0;
}
