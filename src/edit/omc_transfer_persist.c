    ...
    const omc_transfer_res* transfer, const omc_transfer_persist_opts* opts)
{
    ...
    if (transfer->format != OMC_SCAN_FMT_DNG
        || transfer->dng_target_mode != OMC_DNG_TARGET_MINIMAL_FRESH_SCAFFOLD) {
        return 0;
    }
...
static int
omc_transfer_cleanup_requested_by_opts(const omc_transfer_res* transfer,
                                       const omc_transfer_persist_opts* opts)
{
    ...
    return sidecar_base != (const char*)0 && sidecar_base[0] != '\0';
}

static omc_status
omc_transfer_maybe_set_cleanup_path(const omc_transfer_res* transfer,
                                    const omc_transfer_persist_opts* opts,
                                    omc_arena* meta_out,
                                    omc_transfer_persist_res* out_res)
{
    ...
    status = omc_transfer_build_sidecar_path(sidecar_base, meta_out,
    ...
    if (omc_transfer_path_exists(
            (const char*)omc_arena_view(meta_out,
                                        out_res->xmp_sidecar_cleanup_path)
                .data)) {
        return omc_transfer_set_message(
            meta_out, &out_res->xmp_sidecar_cleanup_message,
...
omc_transfer_persist_opts_init(omc_transfer_persist_opts* opts)
{
    ...
    opts->destination_sidecar_mode = OMC_TRANSFER_DEST_SIDECAR_PRESERVE_EXISTING;
}
...
omc_transfer_persist(const omc_u8* edited_bytes, omc_size edited_size,
                     ...
                     const omc_transfer_persist_opts* opts, omc_arena* meta_out,
                     omc_transfer_persist_res* out_res)
{
    ...
    out_res->xmp_sidecar_requested = transfer->writeback_mode
    ...
    if (omc_transfer_cleanup_requested_by_opts(transfer, opts)) {
        out_res->xmp_sidecar_cleanup_requested = 1;
    }
    ...
    if (out_res->xmp_sidecar_requested) {
        ...
        if (sidecar_base == (const char*)0 || sidecar_base[0] == '\0') {
            ...
            if (transfer->sidecar.status != OMC_XMP_DUMP_OK) {
                ...
                sidecar_path_view = omc_arena_view(meta_out,
                                                   out_res->xmp_sidecar_path);
