                                      ...
                                      omc_transfer_package_io_res* out_res)
{
    ...
    omc_bmff_write_item writes[OMC_TRANSFER_BMFF_MAX_REF_ITEMS];
    omc_bmff_write_res rewrite_res;
    omc_u32 i;
    omc_status status;
    ...
    for (i = 0U; i < plan.item_count; ++i) {
        writes[i].payload = items[i].chunk->bytes;
        if (items[i].mime_xmp) writes[i].family = OMC_BMFF_FAMILY_XMP;
        else if (items[i].item_type == OMC_FOURCC('E','x','i','f')) writes[i].family = OMC_BMFF_FAMILY_EXIF;
        else if (items[i].item_type == OMC_FOURCC('j','u','m','b')) writes[i].family = OMC_BMFF_FAMILY_JUMBF;
        else writes[i].family = OMC_BMFF_FAMILY_C2PA;
    }
    status = omc_bmff_rewrite(target_bytes, target_size, writes, plan.item_count,
                             0U, plan.has_icc_profile, plan.icc_profile, 1,
                             out_bytes, &rewrite_res);
    out_res->status = rewrite_res.status;
    if (status != OMC_STATUS_OK || out_res->status != OMC_TRANSFER_OK) return status;
                              ...
                              omc_arena* out, omc_transfer_status* out_status)
{
    ...
    omc_status status;
    if (out_status == NULL) return OMC_STATUS_INVALID_ARGUMENT;
    bytes.data = profile; bytes.size = profile_size;
    status = omc_bmff_rewrite(file_bytes, file_size, NULL, 0U, 0U, 1,
                             bytes, 0, out, &res);
